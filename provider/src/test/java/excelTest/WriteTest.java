...
import util.CustomCellWriteHandler;
import util.CustomSheetWriteHandler;
...
public class WriteTest {
    ...
    public void complexHeadWrite() {
        ...
        // 这里 需要指定写用哪个class去读，然后写到第一个sheet，名字为模板 然后文件流会自动关闭
        EasyExcel.write(fileName, ComplexHeadData.class).sheet("模板").doWrite(data2());
    }
    ...
    public void converterWrite() {
        ...
        // 这里 需要指定写用哪个class去读，然后写到第一个sheet，名字为模板 然后文件流会自动关闭
        EasyExcel.write(fileName, ConverterData.class).sheet("模板").doWrite(data2());
    }
    ...
    private List<DemoData> data() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            data.setId((long) i);
            data.setName(new Date().toString());
            data.setAge(56);
            list.add(data);
        }
        return list;
